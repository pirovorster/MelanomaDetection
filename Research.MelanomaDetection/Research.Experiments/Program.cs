...
namespace Research.Training
{
	class Program
	{
		...
		private static IEnumerable<string> Features
		{
			get
			{
				return
		...
		static void Main(string[] args)
		{
			...
			{
				IEnumerable<string> randomizedMelanomas = allMelanomas.Shuffle();
				IEnumerable<string> randomizedBenigns = allBenigns.Shuffle();
			...
			using (StreamWriter textWriter = new StreamWriter(string.Format("{0}.txt", "Experiments"), true))
			{
				...
				for (int testIndex = 0; testIndex < 10; testIndex++)
				{
					...
					int bestTruePositives = 0;
					int bestFalseNegatives = 0;
					int bestTrueNegatives = 0;
					int bestUnknowns = 0;
					...
					for (int repeat = 0; repeat < 5; repeat++)
					{
						...
						double[] trainedWeights = trainer.TrainWeights(
							new ParticleSwarm(inputNodes * hiddenNodes + hiddenNodes * outputNodes + hiddenNodes + outputNodes, 500, 1000, -10, 10, 0.729, 1.49445, 1.49445, 0.05, 0.02), GetNetworkFactory(inputNodes, hiddenNodes, outputNodes), classifierTrainingItems);
...

						{
							...
							if ((truePositives + trueNegatives) / (double)(falsePositives + truePositives + falseNegatives + trueNegatives + unknowns) > (bestTruePositives + bestTrueNegatives) / (double)(bestFalsePositives + bestTruePositives + bestFalseNegatives + bestTrueNegatives + bestUnknowns))
							{
								bestFalsePositives += falsePositives;
								bestTruePositives += truePositives;
								bestFalseNegatives += falseNegatives;
								bestTrueNegatives += trueNegatives;
					...
					textWriter.WriteLine("False Positives: {0}", bestFalsePositives);
					textWriter.WriteLine("True Positives: {0}", bestTruePositives);
					textWriter.WriteLine("False Negatives: {0}", bestFalseNegatives);
					textWriter.WriteLine("True Negatives: {0}", bestTrueNegatives);
					textWriter.WriteLine("Unknowns: {0}", bestUnknowns);
					textWriter.WriteLine("Accuracy: {0}", (bestTruePositives + bestTrueNegatives) / (double)(bestFalsePositives + bestTruePositives + bestFalseNegatives + bestTrueNegatives + bestUnknowns));
					textWriter.WriteLine();
				}
